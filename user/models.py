...
class User(AbstractBaseUser):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.email is not None:
            if User.objects.filter(email=self.email).exclude(username=self.username).exists():
...
def create_admin_user():
    ...
    if User.objects.filter(default_superuser=True).exists():
        ...
        if User.objects.filter(email=settings.ADMIN_MAIL).exists():
        ...
        if username_unique_fail or email_unique_fail:
            ...
            print(f'{Fore.RED}Could not create default superuser account:')

            if username_unique_fail:
                print('Username already exists on non-superuser account.', end='')

            if email_unique_fail:
                print('E-Mail already exists on non-superuser account.', end='')
